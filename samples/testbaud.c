...
void sendMessage(int SPI, int Reg, int Data); 
void listenMessage();
...
int main(int argc, char*argv[])
{
  ...
  /* DO LOOP HERE */
  //CNF1 = 0xC5;  // baud 83.333kbps
  //CNF2 = 0xBA;
  //CNF3 = 0x07;

    //Set config mode
  sendMessage(0x02, 0x0f, 0x80); //Config mode
  listenMessage();
  sendMessage(0x03, 0x0f, 0x00); //Config mode read
  listenMessage();

  //Configure phase segments
  sendMessage(0x02, 0x29, 0xfa); //CNF2
  listenMessage();
  sendMessage(0x03, 0x29, 0x00); //CNF2 read
  listenMessage();
  sendMessage(0x02, 0x28, 0x07); //CNF3
  listenMessage();
    sendMessage(0x03, 0x28, 0x00); //CNF3 read
  listenMessage();

  //Configure BRP
 // sendMessage(0x03, 0x2a, 0xc5);  // baud 83.333 kbps
  //sendMessage(int Mode, int SPI, int Reg, int Data);
  //sendMessage(int Mode, int SPI, int Reg, int Data);
  //listenMessage()
...
uint8_t buffer[64];
  ...
  buffer[60] = SPI;   /* SPI command = Write.   */
  buffer[61] = Reg;   /* Register = CANCTRL     */
  buffer[62] = Data;      /* Data = 0 (normal mode) */
