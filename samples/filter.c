...
/* Set config mode and speed */
//memset (buffer, 0, 64);
//buffer[58] = 1;     /* CANCTRL = Config mode */
//buffer[60] = 0x02;  /* SPI Write */
//buffer[61] = 0x0f;  /* Register CANCTRL */
//buffer[62] = 1;  /* Config mode */




  /* We can now send and receive messages. For example, set speed. */
//  memset(buffer, 0, 64);
//  buffer[58] = 0x80;      /* CANCTRL = Config mode. */
//  buffer[60] = 0x02;   /* SPI command = Write.   */
//  buffer[61] = 0x2a;   /* Register = CNF1     */
//  buffer[62] = 0x0b;      /* Data = 83.333kbps */
...
/* Set to normal mode and read the speed setting */
//memset(buffer, 0, 64);
// buffer[58] = 0x60;  /* Normal mode */
//buffer[60] = 0x02;  /* SPI Read */
//buffer[61] = 0x0f;  /* Register CANCTRL */
//buffer[62] = 0x60;  /* Listen Only */

//res = libusb_interrupt_transfer(handle, 1, buffer, 64, &numBytes, 100);

//if (res == 0)
//{
//  printf("Sent read request for speed setting\n");
//}
//else
//{
...
char readid;
  ...
  while(buffer[index] & 0x80)
  {
    ...
    else /* standard 11 bit identifier */
    {
      ...
      ++index;
      canID |= ((buffer[index] & 224) >> 5);
      ++index;
    }

//printf("%x\n", canID);
  
//itoa(canID,readid,16); 

//snprintf(readid, 8, "%d", canID);
...
//    if (buffer[62] == 0x07)
//    {
//      printf("Bus speed setting is 125kbps\n");
//    }
//    if (buffer[62] == 0x0b)
//    {
//      printf("Bus speed setting is 83.333kbps\n");
//    }
//  }
}
